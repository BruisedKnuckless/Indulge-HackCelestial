...
import {
  ShoppingBag, MessageSquare, Receipt, Star, Building2, FileText, Radio,
} from 'lucide-react';
      ...
      {!feed.length ? (
        ...
        <ol className="bg-surface-alt border border-line rounded-xl divide-y divide-line/60 overflow-hidden">
          {feed.map((item) => {
            ...
            return (
              <li key={`${item.kind}-${item.id}`} className="flex items-start gap-3 px-4 py-3 hover:bg-surface-sunk/40 transition-colors">
