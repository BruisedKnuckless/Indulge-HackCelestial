...
// Standard Google Maps teardrop vector pin path (spaced explicitly for Google Maps internal SVG parser)
const PIN_SVG_PATH =
...
function useGoogleMapsLoader() {
  ...
  useEffect(() => {
    ...
    if (existingScript) {
      // Poll briefly if script is already present but Map constructor isn't ready yet
 ...
 * Shows an informational message — no fake radar visualization.
 */
...
export default function GoogleMap({
  ...
  const { isLoaded, hasKey, loadError, authError } = useGoogleMapsLoader();
  ...
  }, [isLoaded, userCoords, radiusKm, onSelect]);
  ...
  // Fallback when key is missing or script network request failed completely (e.g. offline)
  if (!hasKey || (loadError && !isLoaded)) {
    return (
      <div className="relative w-full h-full min-h-[380px] rounded-xl overflow-hidden border border-line shadow-sm">
        <NoMapKeyFallback userCoords={userCoords} radiusKm={radiusKm} isAuthError={false} />
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full h-full min-h-[380px] rounded-xl overflow-hidden border border-line shadow-sm">
      ...
      {/* Informative banner if API key requires Maps JavaScript API activation in Google Cloud */}
